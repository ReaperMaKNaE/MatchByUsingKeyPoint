...
def ComparePixelLocation(inputImage, thresholdValue = 20, compareList):
    # Call Image
    ...
    checkDefect = np.zeros(len(triangleForNormalImage))
    defectLocation = []

    for i in range(len(triangleForNormalImage)):
        for j in range(len(compareList)):
            XdistanceBetweenTwo = triangleForNormalImage[i][0] - compareList[j][0]
            YdistanceBetweenTwo = triangleForNormalImage[i][1] - compareList[j][0]

            distanceBetweenTwo = math.sqrt( XdistanceBetweenTwo ** 2 + YdistanceBetweenTwo ** 2)

            if distanceBetweenTwo < 5 :
                checkDefect[i] = 1
                defectLocation.append(triangleForNormalImage[i])

    for i in range(len(triangleForNormalImage)) :
        if checkDefect[i] == 0 :
            print('Defect is Detected! Location is : ', defectLocation)
            break

    for i in range(len(defectLocation)) :
        cv2.circle(defectImage, (int(float(defectLocation[i][0])), int(float(defectLocation[i][1]))), 10, (0,0,255), 3)
