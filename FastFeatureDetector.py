...
def detectDefectUsingThreshold(firstImage, secondImage, thresholdValue1 = 20, thresholdValue2 = 30):
    defect = 0
    popListDistance = 10
    threshold1 = 130
    threshold2 = 150
    ...
    while(defect == 0):
        ...
        for i in range(len(triangleForDefectImage)):
            cv2.circle(imgcheck_defect, (int(float(triangleForDefectImage[i][0])), int(float(triangleForDefectImage[i][1]))), 10, (0,0,255), 3)
    ...
    if(defect == 0):
        print('there are no defect')
        return threshold1+5, threshold2+5
    else:
